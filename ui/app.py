...
class App(UserControl):
    ...
    def login(self, browser, login_method) -> str:
        if login_method == 'cookie':
            ...
            self.client.getQrcode(browser)
            dialog = AlertDialog(
                title=Text("请扫码登录"),
                content=Image(
                    src=BASE_FILEPATH.joinpath('resources', 'qrcode.png'),
            ...
            dialog.open = True
            self.page.update()
            flag = isRedirect(self.client.driver, refresh=False, timeout=30)
            dialog.open = False
            BASE_FILEPATH.joinpath('resources', 'qrcode.png').unlink()
            self.page.update()
            if flag:
                cookie = self.client.driver.get_cookie(name='_t')
                self.client.saveCookie(cookie['name'], cookie['value'])
                return 'succ'
            else:
                return 'timeout'
