...
Postdata = {
    ...
    "reply": 0,  # int
    "actday": 0,  # int
    "replied_user": [],  # list[tuple[gUser, int]]
    "reply_user": [],  # list[tuple[gUser, int]]
    "topPostingDay": [],  # list
    "daylist": [],  # list
    "monlist": [],  # list
    # 'category': None # dict
...
class PostCard(Card):
    ...
    def cal_height(self, starty: int) -> int:
        self.y1 = starty
    ...
    def draw_card(self):
        ...
        text = "1. 共发贴：{}，分布在 {} 个话题中".format(
            self.data["post"], len(self.data["topic"])
        )
        size = self.text(size[0] + 20, size[3] + self.gap, text, font=self.textfont)
        text = "2. 共收到回复：{} 条".format(str(self.data["reply"]))
        size = self.text(
            size[0], size[3] + self.gap, text, font=self.textfont, multiline=True
        )
        if self.data["post"] > 0:
            size = self.text(
                size[0], size[3] + self.gap, "3. 发帖最多的3个话题:", font=self.textfont
            )
            tmpx = size[0] + 20
            tmpx1 = size[0]
            for i, topic in enumerate(self.data["topic"]):
                size = self.text(
                    tmpx,
                    size[3] + self.gap,
                    f"• {self.data['topic'][topic]['topic_title']}",
                    font=self.textfont,
                    omit=True,
                    edge=650,
                )
            ...
            size = self.text(
                tmpx1, size[3] + self.gap, text, font=self.textfont, multiline=True
            )
            M = max(self.data["daylist"])
            idx = self.data["daylist"].index(M)
            text = "5. 一年中，你在 {} 至 {} 时发帖最多，共发 {} 帖".format(
                idx, idx + 1, M
            )
            ...
            if len(self.data["reply_user"]) > 0:
                self.minicard(
                    self.data["reply_user"],
                    "最多回复至",
                    (size[0] + int(0.5 * self.width), size[3] + self.gap),
                    "Reply",
                    "r",
                )
