...
class Post():
    ...
    def getTime(self, unit: Literal['year', 'mon', 'day', 'hour']) -> int:
        ...
        ddt = datetime.strptime(self.time,'%Y-%m-%dT%H:%M:%S.%fZ').replace(tzinfo=ZoneInfo('UTC'))
...
def getPost(MainUser: mUser, headers: dict, year, info) -> list:
...
def getReply(MainUser: mUser, headers: dict, year, info) -> tuple[int, list]:
    flag = 0
    offset = 0
    count = 0
    reply_user_list = {}
    ...
    for i in range(len(reply_user_list)):
        user = getUser(headers=headers, mode='g', username=reply_user_list[i][0])
...
def getDetails(x: dict[list], headers: dict, info, my) -> tuple[list, int, dict, dict]:
    '''
    x = {
        topic_id: {
            'topic_title': str,
    ...
    emoji_dict = {}
    emoji_user_dict = {}
    urlList = makeUrlList(x)
    total = len(urlList)
    M = 20
    for i in range(0, total, M):
        subList = urlList[i:i+M]
        posts = []
